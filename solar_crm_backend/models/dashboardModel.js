...
const getStatusBreakdown = async (managerId, db = defaultDb) => {
  const query = `
...
const getTeamPerformance = async (managerId, db = defaultDb) => {
  const query = `
    SELECT 
...
const getPendingToday = async (managerId, db = defaultDb) => {
  const query = `
...
const getConvertedToday = async (managerId, db = defaultDb) => {
  const query = `
...
const getRecentActivity = async (managerId, db = defaultDb) => {
  const query = `
...
const getTodaysFollowupsList = async (managerId, db = defaultDb) => {
  const query = `
    SELECT 
      l.id, 
      l.lead_code, 
...
const getOverdueFollowupsCount = async (managerId, db = defaultDb) => {
  const query = `
...
const getSalesSummary = async (userId, db = defaultDb) => {
  const query = `
    SELECT
...
const getSalesActivityTimeline = async (userId, limit = 20, db = defaultDb) => {
  const query = `
...
const getSalesStatusBreakdown = async (userId, db = defaultDb) => {
  const query = `
...
const getSalesFollowupsList = async (userId, db = defaultDb) => {
  const query = `
...
const getAdminOverview = async (db = defaultDb) => {
  const query = `
    SELECT
...
const getAdminStatusBreakdown = async (db = defaultDb) => {
...
const getAdminSourcePerformance = async (db = defaultDb) => {
  const [rows] = await db.query(`
    SELECT lead_source,
      COUNT(*) AS total,
...
const getAdminMonthlyTrend = async (db = defaultDb) => {
  ...
  const now = new Date();
  for (let i = 5; i >= 0; i--) {
    const key = `${now.getFullYear()}-${String(now.getMonth() + 1 - i).padStart(2, "0")}`;
...
const getAdminRecentLeads = async (limit = 6, db = defaultDb) => {
  const [rows] = await db.query(
    `
...
const getAdminActivityFeed = async (limit = 15, db = defaultDb) => {
  const [rows] = await db.query(
    `
