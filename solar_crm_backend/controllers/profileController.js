...
const getMyProfileController = async (req, res) => {
    try {
        const profile = await getMyProfile(req.user.id, req.db);
...
const updateMyProfileController = async (req, res) => {
    try {
        ...
        if ((await checkUsernameExistsForUpdate(username, req.user.id, req.db)).length > 0) {
            return res.status(400).json({
                success: false,
        ...
        if ((await checkEmailExistsForUpdate(email, req.user.id, req.db)).length > 0) {
            return res.status(400).json({
                success: false,
        ...
        if ((await checkPhoneExistsForUpdate(phone, req.user.id, req.db)).length > 0) {
            return res.status(400).json({
                success: false,
